...
import { tennisAssessmentSchemas } from "~/models"; // Changed import
import { AssessmentDbRepository } from "~/repositories/assessmentDbRepository";

export const handlerConfig = defineEventHandlerConfig({
  name: "getTennisAssessmentV1List", // Changed name
  response: response(
    type({ assessments: [tennisAssessmentSchemas[1], "[]"] }), // Changed schema
    {
      stream: false,
    },
  ),
  ...
  pipe(
    ...
    Effect.bind(
      ...
      ({ assessmentDbRepository, userId }) =>
        assessmentDbRepository.getTennisAssessmentV1Performed(userId), // Changed method call
    ),
