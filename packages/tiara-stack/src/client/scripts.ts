...
const createTiaraContents = (config: TiaraConfig<never>): TiaraContentMap => {
  return {
    ...
    effectContext: [
      ...
      ``,
      `import { dependenciesLive } from "../server/layers/dependencies";`,
      ``,
      `export const { effectContext, hooks } = /*@__PURE__*/ Effect.runSync(`,
      `  /*@__PURE__*/ getEffectContextHooks({ layer: dependenciesLive }),`,
      `);`,
      ``,
    ].join("\n"),
    effectEventHandler: [
      ...
      `import { CoercedResponseType, EventHandlerConfig } from "tiara-stack/config";`,
      `import { createEffectEventHandler, EffectEventHandlerWrapper } from "tiara-stack/utils/effectEventHandler";`,
      ``,
      ...
      `  /*@__PURE__*/ createEffectEventHandler(effectContext);`,
      ``,
      `/*@__NO_SIDE_EFFECTS__*/`,
      ...
      `  >,`,
      `  R extends Layer.Layer.Success<typeof dependenciesLive>,`,
      `>(config: C): EffectEventHandlerWrapper<C, R> => /*@__PURE__*/ _effectEventHandler<C, R>(config);`,
      ``,
      `export {`,
      `  EventContext,`,
      `  EventParamsContext,`,
    ...
    models: [
      ...
      `import { EventHandlerResponseValidatorType, EventHandlerQueryValidatorType, EventHandlerBodyValidatorType, EventHandlerRouterValidatorType } from "tiara-stack/config";`,
      `import { apiRoutes, type ApiRoutes } from "./serverRoutes";`,
      ``,
      `export type ${pascalCase(`${config.name}_client_response_type`)}<K extends keyof ApiRoutes> =`,
      `  EventHandlerResponseValidatorType<ApiRoutes[K]["config"]>;`,
      ``,
      `export type ${pascalCase(`${config.name}_client_query_type`)}<K extends keyof ApiRoutes> =`,
      `  EventHandlerQueryValidatorType<ApiRoutes[K]["config"]>;`,
      ``,
      `export type ${pascalCase(`${config.name}_client_body_type`)}<K extends keyof ApiRoutes> =`,
      `  EventHandlerBodyValidatorType<ApiRoutes[K]["config"]>;`,
      ``,
      `export type ${pascalCase(`${config.name}_client_router_type`)}<K extends keyof ApiRoutes> =`,
      `  EventHandlerRouterValidatorType<ApiRoutes[K]["config"]>;`,
      ``,
